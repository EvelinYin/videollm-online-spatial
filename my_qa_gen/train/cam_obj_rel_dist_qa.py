...
def process_qa(item: Dict[str, Any], seed: int = None) -> List[Dict[str, Any]]:
    """
    Transform a single camera_obj_rel_dist QA item into three question formats.
    ...
    # 2. Extract frame numbers: "in frame 26 of 32"
    ...
    pattern = r"which of these objects +(.*?)\s+is the closest"
    object_match = re.search(pattern, question_text)
    if not object_match:
        return None
    object_names = object_match.group(1)
    ...
    if 0 in question_indices:
        # 1. Past ask Future: Ask at frame_before (-1 to -5), about future frame_asked
        ask_frame_idx = max(1, norm_frame_asked - random.randint(1, 5))
        
        # Calculate asking timestamp
        ...
        results.append({
            ...
            "conversation": [
                {
                    "role": "user",
                    # Changed: "...in frame {frame_asked}..." -> "...at {target_time_str}..."
                    "content": f"Measuring from the closest point of each object at {target_time_str}, which of these objects {object_names} will be the closest to the camera?\n{options_text}",
                    "time": ask_time
                },
                {
                    "role": "assistant",
    ...
    if 1 in question_indices:
        # 2. Current ask Current: Ask at target time
        results.append({
            "video_uid": item.get('scene_name', ''),
            "conversation": [
                {
                    ...
                    "content": f"Measuring from the closest point of each object at current time, which of these objects {object_names} will be the closest to the camera?\n{options_text}",
                    "time": target_time_val
                },
                {
                    "role": "assistant",
    ...
    if 2 in question_indices:
        # 3. Future ask Past: Ask at frame > frame_end (±5)
            ...
            "conversation": [
                {
                    "role": "user",
                    # Changed: "...in frame {frame_asked}..." -> "...at {target_time_str}..."
                    "content": f"Looking back, measuring from the closest point of each object at {target_time_str}, which of these objects {object_names} will be the closest to the camera?\n{options_text}",
                    "time": ask_time
                },
                {
                    "role": "assistant",
...
def process_dataset_parallel(dataset, question_type_list: List[str], output_path: str, num_workers: int = 4, seed: int = None):
    """
    Process camera_displacement questions in parallel and save to JSON.

    Args:
        dataset: The dataset to process
        question_type_list: List of question types to filter for
...
if __name__ == "__main__":
    ...
    question_types = ['camera_obj_rel_dist_v1', 'camera_obj_rel_dist_v2', 'camera_obj_rel_dist_v3']

    # Process and save
    output_file = "./datasets/vlm3r_ours/my_qa/train/camera_obj_rel_dist_reformatted.json"
    process_dataset_parallel(ds, question_types, output_file, num_workers=4, seed=SEED)