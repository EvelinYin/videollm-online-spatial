...
def process_qa(item: Dict[str, Any], seed: int = None) -> List[Dict[str, Any]]:
    ...
    # Captures everything starting from "The options are..."
    options_match = re.search(r"(The options are.*)", question_text, re.DOTALL)
    if options_match:
        options_text = options_match.group(1)
    else:
        # Fallback if regex fails (though it shouldn't for this dataset)
        options_text = ""
...
if __name__ == "__main__":
    ...
    # Process and save
    output_file = "./datasets/vlm3r_ours/my_qa/train/camera_movement_direction_reformatted.json"
